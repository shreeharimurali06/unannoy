...
export function CopyButton({
  ...
  async function copyText() {
    ...
    try {
      ...
      setStatus("copied");
      trackEvent("copy_clicked", { tool_slug: toolSlug, length_bucket: getLengthBucket(text.length) });
    } catch {
      setStatus("failed");
    }
  ...
  return (
    ...
    >
      ...
      {status === "copied" ? "Copied. One less tiny problem." : status === "failed" ? "Could not copy. Browser said no." : label}
    </button>
  );
}
